CREATE OR REPLACE PROCEDURE P_custProtocal_Proc IS
...
begin
   ...
   select max(sid) into v_sid from lpt4_xm128_trade.Ac_Cust_Protocal;
  
   for cursor_var in cn loop
     
   v_sid:=v_sid+1;
        
    dbms_output.put_line(cursor_var.cust_no);
    insert into Ac_Cust_Protocal (
     SID, CUST_NO, SIGN_FLAG, PROTOCAL_CODE,
     REC_STAT, CHECK_FLAG, CREATOR, MODIFIER,
     CHECKER, CRE_DT, MOD_DT, STIMESTAMP,
     UPDATED_STIMESTAMP)
    values (
    v_sid, cursor_var.cust_no, '0', '001',
     '0', '1', 'wap', null,
     'wap', '20150820', null, '20-8月 -15 05.02.52.222000 下午',
     '20-8月 -15 05.02.52.222000 下午');
