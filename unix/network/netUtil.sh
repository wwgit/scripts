...
usage(){
  ...
  if  [ X = "$1"X ]; then
	 ...
	 echo "if m=b, option int and dur will be ignored !"
	 ...
	 echo "example: sh netUtil.sh -netstat m=a,int=5,dur=30m,cond=80,cs=listen"
	 exit 1
...
cmdExec(){
  ...
  #echo "before loop, contents in stdinout file:" >&888
  #cat $stdInOut >&888
  ...
  #echo "after execute before return to parent, contents in stdinout file:" >&888
  #cat $stdInOut >&888
...
netstExec(){
  ...
  for item in ${condArr[*]}
   ...
   if [ "$mode"X = "b"X ]; then
      ...
      ns_cmd="$ns_cmd"" -apn"
      #stdRedirec	  
	  cmdOutput=`cmdExec "$ns_cmd" "${query_conds[*]}"`
	  
	  filename=${filename}_basic
	  ret_path=$ret_path/${filename}
	  ret_path=`strAppend $ret_path ${query_conds[0]} "_"`		   
	  ret_path=`strAppend $ret_path ${conn_stats} "_"`          
   ...
   if [ "$mode"X = "a"X ]; then	   
       ...
       do
		   ...
		   #echo "after call, contents in stdinout file:" >&888
		   #cat $stdInOut >&888
		   
		  # echo "contents in cmdOutput:" >&888
		  # echo "$cmdOutput" >&888
		   ...
		   cmdOutput=`echo "$cmdOutput" | awk -v conn_status="$conn_stats" 'BEGIN{
			                                    ...
			                                    END{ print state[conn_status]; }'`
											   
		   #fileName contents handling happens here:
           cmdOutput="totalNum="${cmdOutput}
		   cmdOutput=`strAppend $cmdOutput ${query_conds[0]} ","`
		   cmdOutput=`strAppend $cmdOutput ${conn_stats} ","`
		   cmdOutput=${cmdOutput}","`date "+%H:%M:%S"`
           ...
           #echo "rest time is: "$rest_durs >&888	  		  
       done	   	   	   
   fi
   
   #need to change stdin stdout back call is done.
   #stdRecov
