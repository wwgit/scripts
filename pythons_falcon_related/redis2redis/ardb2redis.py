...
def filter_key(k, st_slot, ed_slot):
	max_slot_len = 16383
	slot = int(crc16.crc16xmodem(k))%max_slot_len
...
def prepare_get_data(config):
	try:
		...
		src_r = redis.StrictRedis(host=src_host, port=src_port, db=0)
		#limit_count = int(config["get_key_limit"])
		#get keys and types
		#keys = src_r.keys('*')
		#all_keys = '*
		keys = src_r.execute_command("keys * limit " + str(config["get_key_limit"]))
		print "before fiter keys count ", len(keys)
		...
		print "after fiter keys count ", len(keys)
		if 0 == len(keys):
			return
...
if __name__ == "__main__":
	try:
		...
		p.add_argument('-c', '--config', default='config.yml', help='config file path,default:config.yml')
		p.add_argument('-v', '--version', action='version', version=VERSION, help='show current script version')
		args = p.parse_args()
			
		if not os.path.exists(args.config):
			print "config path error", args.config
			exit(-1)
